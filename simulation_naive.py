...
raw_cache_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'raw_cache_naive.p')
agents_cache_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'agents_cache_naive.p')
agents_loaded = False
...
if use_raw_cache == '' or use_raw_cache == 'y':  # obtains cached variables from the file data cache
    ...
    (cbg_ids, lda_documents, cbgs_to_households, cbgs_leaving_probs,
     dwell_distributions, poi_type, poi_set) = pickle.load(raw_cache_file)
    ...
    if use_agents_cache == '' or use_agents_cache == 'y':
        ...
        (agents, households, cbgs_to_agents, inactive_agent_ids, active_agent_ids, poi_current_visitors, Ipa_queue_tups,
         Ic_queue_tups, R_queue_tups, quarantine_queue_tups, total_ever_infected) = pickle.load(agents_cache_file)
        agents_loaded = True
    ...
    data = pd.read_csv(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'safegraph-data', 'safegraph_weekly_patterns_v2',
                     'main-file', '{}-weekly-patterns'.format(WEEK), '{}-{}.csv'.format(area, WEEK)),
    ...
    for row in usable_data.itertuples():
        ...
        for cbg in cbgs:
            ...
            if cbg not in cbgs_to_pois:
                cbgs_to_pois[cbg] = []
            cbg_frequency = random.randint(2, 4) if cbgs[cbg] == 4 else cbgs[
                cbg]  # SafeGraph reports POI CBG frequency as 4 if the visit count is between 2-4
            cbgs_to_pois[cbg].extend(
                [place_id] * cbg_frequency)  # generate POIs as "words" and multiply by the amount that CBG visited
    ...
    # cbgs_to_topics = dict(zip(cbg_ids, list(lda_model.get_document_topics(lda_corpus,
    #                                                                       minimum_probability=0))))  # {'510594301011': [(0, 5.5570694e-05), (1, 5.5570694e-05), (2, 5.5570694e-05), (3, 5.5570694e-05), ...], ...}
    # topics_to_pois = [[[tup[0] for tup in givens[1]], [tup[1] for tup in givens[1]]] for givens in
    #                   lda_model.show_topics(formatted=False, num_topics=NUM_TOPICS,
    #                                         num_words=poi_count)]  # [[poi id list, probability dist], [poi id list, probability dist], ...]
    #
    ...
    # for topic, givens in enumerate(
    #         topics_to_pois):  # iterate through each topic to establish chances of leaving per hour
    ...
    #                 hourly_poi_probabilities.append(
    #                     poi_probabilities[idx] * poi_hour_counts[poi_id][hour] / sum(poi_hour_counts[poi_id]))
    ...
    census_population_data = pd.read_csv(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'safegraph-data', 'safegraph_open_census_data', 'data',
                     'cbg_b01.csv'), error_bad_lines=False)
    ...
    census_household_data = pd.read_csv(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'safegraph-data', 'safegraph_open_census_data', 'data',
                     'cbg_b11.csv'), error_bad_lines=False)
    ...
    print(r'The simulated population will be {}% of the real population.'.format(
        adj_sig_figs(100 * total_population / total_real_population)))
    ...
    for date in pd.date_range(WEEK, periods=7, freq='D'):
        print(date)
        social_distancing_data = pd.read_csv(gzip.open(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), 'safegraph-data',
                         'safegraph_social_distancing_metrics', str(date.year), str(date.month).zfill(2),
                         str(date.day).zfill(2),
                         '{}-{}-{}-social-distancing.csv.gz'.format(str(date.year), str(date.month).zfill(2),
                                                                    str(date.day).zfill(2))), 'rb'),
                                             error_bad_lines=False)
    ...
    print('Average percentage of population tracked per day: {}%'.format(
        adj_sig_figs(100 * total_devices_tracked / (total_population * 7))))
    ...
    raw_cache_data = (
    cbg_ids, lda_documents, cbgs_to_households, cbgs_leaving_probs,
    dwell_distributions, poi_type, poi_set)
...
if not agents_loaded:
    ...
    agents = {}  # agent_id: [infected_status, (current poi, expiration time) or None, home_cbg_code]
    ...
    active_agent_ids = {t: set() for t in
                        range(total_simulation_time)}  # expiration_time: {(agent_id, poi_id), (agent_id, poi_id), ...}
    poi_current_visitors = {poi: set() for poi_list in lda_documents for poi in
                            poi_list}  # poi id: {agent_id, agent_id, ...}
    ...
    def add_agent(current_cbg, household_id):
        ...
        if not permanently_quarantined:
            ...
            if rand < PROPORTION_INITIALLY_INFECTED:
                ...
                if rand < 0.23076923076:  # 2.754 / (2.754 + 1.928 + 2.662 + 4.590), from exposure gamma distributions
                    ...
                    agent_status = 'Ia'
                    R_queue_tups.append(
                        (round(distribution_of_subclinical.rvs(1)[0] * daily_simulation_time), agent_id))
                    inactive_agent_ids.add(agent_id)
                elif rand < 0.7323278029:  # 1.928 / 4.590 of the remaining value, represents preclinical cases
                    agent_status = 'Ip'
                    Ic_queue_tups.append(
                        (round(distribution_of_preclinical.rvs(1)[0] * daily_simulation_time), agent_id))
                    inactive_agent_ids.add(agent_id)
            ...
            inactive_agent_ids.add(agent_id)
        agents[agent_id] = [0, agent_status, parameter_2, current_cbg]
        ...
        for idx, cbg_household_count in enumerate(cbgs_to_households[current_cbg]):
            ...
            for _ in range(cbg_household_count):
                ...
                for _ in range(current_household_size):
                    add_agent(current_cbg, household_id)
    ...
    agents_cache_data = (
    agents, households, cbgs_to_agents, inactive_agent_ids, active_agent_ids, poi_current_visitors, Ipa_queue_tups,
    Ic_queue_tups, R_queue_tups, quarantine_queue_tups, total_ever_infected)
...
print(
    'Total ever infected: {} ({}%)'.format(total_ever_infected, adj_sig_figs(100 * total_ever_infected / len(agents))))
...
#             minimum_list_index -= 1
#         aux_dict[topic][t] = [topics_to_pois_by_hour[topic][t][0][minimum_list_index:],
#                               topics_to_pois_by_hour[topic][t][1][
#                               minimum_list_index:]]  # [poi ids list, poi probabilities list for numpy.choice]
#         aux_dict[topic][t][1] = numpy.array(aux_dict[topic][t][
#                                                 1])  # converts poi probabilities list at aux_dict[topic][t][1] from a list to a numpy array for numpy.choice
#         aux_dict[topic][t][1] /= aux_dict[topic][t][
#             1].sum()  # ensures the sum of aux_dict[topic][t][1] is 1 for numpy.choice
...
#         topics_to_pois_by_hour[topic][t][1] = numpy.array(
#             topics_to_pois_by_hour[topic][t][1][:minimum_list_index] + [prob_sum])
...
def select_active_agents(t):  # puts agents into POIs whose visits start at at time t
    ...
    for agent_id in inactive_agent_ids:
        ...
        if random.random() < cbgs_leaving_probs[agents[agent_id][3]] / SIMULATION_TICKS_PER_HOUR * 0.004: # removed topic_hour_distributions
            destination = random.choice(tuple(poi_set))
        ...
        if poi not in closed_pois and len(poi_current_visitors[poi]) > 1:
            ...
            for agent_idx, agent_id in enumerate(poi_agents):
                if agents[agent_id][1] == 'Ia' or agents[agent_id][1] == 'Ip':
                    ...
                    poi_infect(np.random.choice(possible_agents, max_interactions), current_time,
...
def household_transmission(current_time):  # simulates a day's worth of of household transmission in every household
    for household in households.values():
        base_transmission_probability = .091 / 4.59 if len(
            household) < 6 else .204 / 4.59  # https://www.thelancet.com/journals/laninf/article/PIIS1473-3099(20)30471-0/fulltext
...
def quarantine(agent, days,
               current_time):  # agents, number of days in quarantine, current day => agents cannot leave house during quarantine
...
def set_agent_flags(current_time):  # changes agent infection status
    ...
    while Ipa_queue and Ipa_queue.queue[0][
        0] <= current_time + 1:  # https://www.nature.com/articles/s41586-020-2196-x?fbclid=IwAR1voT8K7fAlVq39RPINfrT-qTc_N4XI01fRp09-agM1v5tfXrC6NOy8-0c
        key = Ipa_queue.get()[1]
...
def report_status():  # prints status
    ...
    print('New cases per day: {}'.format(
        list(zip([i for i in range(len(infection_counts_by_day))], infection_counts_by_day))))
    print(r'Total ever infected: {} ({}% of the total population)'.format(total_ever_infected, adj_sig_figs(
        100 * total_ever_infected / len(agents))))
    print()
...
for current_time in range(total_simulation_time):
    ...
    if not (current_time + 1) % SIMULATION_TICKS_PER_HOUR:  # if the next tick marks the start of an hour
        print('Hour {} ({}) complete.'.format(int(current_time // SIMULATION_TICKS_PER_HOUR % 24),
                                              int(current_time / SIMULATION_TICKS_PER_HOUR)))
        print_elapsed_time()
