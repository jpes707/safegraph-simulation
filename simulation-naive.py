...
NUM_TOPICS = 1  # always 1 for naive
...
print_elapsed_time()
outfile_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'console-outputs', 'naive-{}-{}.txt'.format(area, WEEK))
...
def get_dwell_time(dwell_tuple):  # given a cached tuple from the dwell_distributions dictionary for a specific POI, return a dwell time in ticks
    return SIMULATION_TICKS_PER_HOUR
